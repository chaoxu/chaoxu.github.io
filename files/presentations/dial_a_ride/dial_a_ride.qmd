...
How: One can show the coefficients of each fundamental cycle is at most $|n^2|$, so we reduce the search space to $(2n^2+1)^{p}$.
 
## Results
:::{style="background: #fff1e8; padding:10px; margin:10px"}
 For a graph with at most $k$ vertices with degree at least $3$, and circuit rank $p$, the running time of the algorithm is $O((2n^2+1)^{p} 2^k MST(n))$.
:::
...
:::{.fragment style="background: #fff1e8; padding:10px; margin:10px"}
 SCP can be solved in $O(n^4 MST(n))$ for $\boxminus$.
:::

:::{.fragment style="background: #fff1e8; padding:10px; margin:10px"}
 SCP problem can be solved in $O(n^2 MST(n))$ for $\bigcirc$.
:::
