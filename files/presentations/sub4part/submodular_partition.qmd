...
## Computer search!
 ...
 - Assumption: For any $\mathcal{X}=\{X_1,X_2,X_3\}$ min-$3$-partition and any $\mathcal{Y}=\{Y_1,Y_2,Y_3,Y_4\}$ min-$4$-partition. There exists a min-4-partition can be obtained by grouping $Z_{ij} = X_i\cap Y_j$. 
...
- $f(A)+f(B)\leq f(A\cup B)+f(A\cap B)$ for all $A,B\subseteq V$.
- $x_A+x_B\leq x_{A\cup B}+x_{A\cap B}$.
...
## Encode min-$k$-partitions
 ...
 - Encode $\mathcal{X}$ as a min-$3$-partition. So $\sum_{X\in \mathcal{X}} f(X)\leq \sum_{X\in \mathcal{X'}} f(X)$ for all $3$-partition $\mathcal{X}'$ ($86526$ constraints)
 - Similarly for min-$4$-partition, and set its value to be $1$ ($611501$ constraints). 
...
## Remaining LP
 ...
 - The theorem is true if $z=1$ for every choice of $\mathcal{X}$ and $\mathcal{Y}$ ($611501\times 86526 = 52910735526$, but use symmetry and other magic, we can cut it down to $23$).
...
## Open problems
 ...
 - Every min $k$-partition has at least $\lceil k/2\rceil$ parts contained in some parts of a min $k+1$-partition?
 - What are the redundant constraints for the LP? 
